...
- P0.1 Persistenz/Recovery-Hardening ist abgeschlossen: Restore-Fehlerpfade differenzieren jetzt explizit Datei-fehlt/JSON-defekt/IO/Schemafehler und liefern immer `Next step`-Hinweise; zusaetzlich werden Audio-Mix-States (`trackAudioMutedById`, `trackAudioSoloById`) beim Restore zur Konsistenz zwingend zurueckgesetzt.
- P0.2 Workflow-Studio-Hardening ist abgeschlossen: Send nutzt jetzt zwingenden Payload-Preflight, Fehler werden einheitlich als Code+Message+Next-Step im UI gezeigt, Recent Runs sind gegen Event-Races gehaertet (Upsert/Out-of-order-Guard/stabile Sortierung + Persistenz), und Output-Import behandelt Duplikate/unsupported/view-/netzwerknahe Teilfehler transparent mit Reason-Codes.
...
## Aufgabenjournal
- 2026-03-09: P0.2 `Workflow Studio Produktionssicherheit` abgeschlossen. `WorkflowStudioView` wurde auf strukturierte Workflow-Fehler (`code`, `message`, `nextStep`) umgestellt; Send fuehrt jetzt vor Queue einen verpflichtenden Preview-Preflight aus und blockiert mit klarer Fehlerrueckmeldung bei Validation-/Template-/Placeholder-Problemen statt still zu scheitern. Output-Import ist gehaertet (Duplicate-Guard im UI + persistenter Duplicate-Tag in Main, Teilfehler-Summary mit Reason-Codes, klare Next-Step-Hinweise). `applyComfyRunEventState` wurde fuer Race-Stabilitaet ausgebaut (Run-Upsert bei unbekannten IDs, Out-of-order-Schutz, stabile Sortierung, kein Workflow-Verlust bei `workflowId=unknown` Events). `comfySlice` persistiert Recent-Runs und auto-importierte Outputs in `localStorage`. Main-Comfy-Service liefert fuer Queue/Preview/Cancel codierte Fehlermeldungen mit Next-Step. Gates komplett gruen nach Fix-Reruns: `npm run lint`, `npm run typecheck`, `npm run build`, `npm run validate:workflows`, `npm run validate:preset-storage --workspace @ai-filmstudio/desktop`. Workflow-nahe Smoke-Ergebnisse dokumentiert (Catalog/Send/Run-State per automatisiertem Service-/Store-Smoke; Dev-Boot erfolgreich).
- 2026-03-09: P0.1 `Hardening Persistenz + Recovery` abgeschlossen. `main/index.ts` liefert fuer Last-Session- und Autosave-Restore jetzt klare Fehlertexte mit naechstem Handlungsschritt (Datei fehlt, defektes JSON, IO/Permission, invalides Projekt-Schema). Store-Restore wurde auf `Promise<boolean>` gehaertet und surfacet Restore-Fehler sichtbar, waehrend informative Erststart-Meldungen (`No previous session found.`) nicht als Fehlerbanner eskalieren. `toProjectState(...)` setzt Audio-Mix-Maps bei Restore auf leer zur Vermeidung inkonsistenter Track-Mute/Solo-Reste. In `SettingsView` wurde ein Recovery-Bereich mit direkten Restore-Aktionen ergaenzt; der Autosave-Dialog zeigt Restore-Fehler inline statt nur implizit. Gate-Checks komplett gruen: `npm run lint`, `npm run typecheck`, `npm run build`, `npm run validate:workflows`; zusaetzlich Smoke-nahe Checks: `npm run validate:preset-storage --workspace @ai-filmstudio/desktop` sowie Dev-Boot-Prozesscheck (`DEV_SMOKE:RUNNING`/`STOPPED`).
